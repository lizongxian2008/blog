 ...
 * @returns
 */
function reomveHtml(str) {
	var re=/<[^>]+>/gi;
	return str.replace(re,"");
};
function replaceAll(str,s1,s2){ 
	return (new String(str)).replace(new RegExp(s1,"gm"),s2);//g=global, m=multiLine 
};
(function($) {
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			rpOptions : [15, 30, 50],
			title : false,
			pagestat : '显示记录：第 {from} 至 {to} 条，总记录数：{total} 条。',
			pagetext : '页码',
			outof : '/',
			findtext : '查询',
			procmsg : '数据加载中, 请稍候 ...',
		...
		var g = {
			...
			addData : function(data) {
				...
				if (p.preProcess) {
					...
					if(data == null){
						data = {rows:[], page:0, total:0};
					}
				}
				if (p.dataType == 'json' && data && data.data) {
					data = data.data;
				}
				...
				if (p.dataType == 'json') {
					$.each(data.rows,function(i, row) {
						...
						$('thead tr:first th', g.hDiv).each(function() {
							...
							td.align = this.align;
							if(i==0){
								_width += p.colModel[idx].width;
							}
							var allowHtml = p.colModel[idx].html;
							if(p.colModel[idx].name=="operate"){
								allowHtml = true;
							}
							if(p.colModel[idx].checkbox){
								var value = "";
								if (typeof row.cell == 'undefined') {
									value = row[p.colModel[idx].name];
								} else {
									if (typeof row.cell[idx] != "undefined") {
										value = (row.cell[idx] != null) ? row.cell[idx] : '';
									} else {
										value = row.cell[p.colModel[idx].name];
									}
								}
								td.innerHTML = '<input type="checkbox" class="chkbox" name="'+p.colModel[idx].name+'" value="'+value+'" />';
							}else if(p.colModel[idx].radio){
								...
								if (value != -99){
									td.innerHTML = '<input type="radio" class="chkbox" name="'+p.colModel[idx].name+'" value="'+value+'" />';
								}else{
									td.innerHTML = '';
								}
								...
								if (typeof row.cell == 'undefined') {
									var html = row[p.colModel[idx].name];
									if(allowHtml){
										td.innerHTML = html;
									}else{
										$(td).text(replaceAll(html,"\n"," "));
									}
									if (p.colModel[idx].showtitle) {
										var _title = p.colModel[idx].titlekey?row[p.colModel[idx].titlekey]:row[p.colModel[idx].name];
										if(_title && "null"!=_title){
											_title = replaceAll(reomveHtml(_title),"\"","\\\"");
											$(td).attr("title",_title);
										}
									}
								} else {
									if (typeof row.cell[idx] != "undefined") {
										var html = (row.cell[idx] != null) ? row.cell[idx] : '';										
										if(allowHtml){
											td.innerHTML = html;
										}else{
											$(td).text(replaceAll(html,"\n"," "));
										...
										if (p.colModel[idx].showtitle) {
											var _title = (row.cell[idx] != null) ? row.cell[idx] : '';
											var title = p.colModel[idx].titlekey ? row[p.colModel[idx].titlekey] : _title;
											if(title && "null"!=title){
												title = replaceAll(reomveHtml(title),"\"","\\\"");
												$(td).attr("title",title);
											}
										}
									} else {
										var html = row.cell[p.colModel[idx].name];										
										if(allowHtml){
											td.innerHTML = html;
										}else{
											$(td).text(replaceAll(html,"\n"," "));
				...
				if ($.browser.msie && $.browser.version < 7.0) {
					if (p.height == 'auto' && $("table",g.bDiv).width() > $(g.bDiv).width()){
						$(g.bDiv).height($("table",g.bDiv).height()+20);//IE6自适应高度，隐藏纵向滚动条
					}
				}
			},
			...
			populate : function() {
				...
				if(p.useTotal){
					param[param.length] ={ name : 'total', value : p.total}
				}
				$.ajax({
					type : p.method,
					url : p.url,
		...
		if (p.colModel) {
			...
			for ( var i = 0; i < p.colModel.length; i++) {
				...
				if(cm.checkbox){
					th.innerHTML = '<input type="checkbox" class="chkbox" name="'+(cm.name+"_checkall")+'" />';
				}else if(cm.radio){
					th.innerHTML = '';
				}
				else{
		...
		$('thead tr:first th', g.hDiv).each(function() {
			...
			if($(this).html().indexOf("checkbox")>0){
				$(this).find(":checkbox").click(function(){
					var checked = $(this).is(":checked");
					var name = $(this).attr("name").replace("_checkall","");
					$(g.bDiv).find(":checkbox[name='"+name+"']").each(function(){
						if(checked){
							$(this).attr("checked","checked");
						}else{
							$(this).removeAttr("checked");
		...
		if (p.usepager) {
			...
			var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'
					+ p.pagetext
					+ ' <input type="text" style="width:30px;height:18px;" value="1" /> '
					+ p.outof
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select style='height:20px;margin-top:2px;' name='rp'>"+opt+"</select></div> <div class='btnseparator'></div>");
