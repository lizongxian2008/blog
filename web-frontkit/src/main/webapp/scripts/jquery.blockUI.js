...
;(function() {"use strict";
	function setup($) {
		...
		function install(el, opts) {
			...
			if (opts.draggable && $.fn.draggable) {//opts.theme &&
				lyr3.draggable({
					handle: '.popbox .popTitle',
					cancel: '.popbox .popTitle .popx'
				});
			}
		...
		function handler(e) {
			...
			var a = target.parents().children().filter('div.blockUI').length === 0;
			var b = target.parents().children().filter('div.pop_overMsgDiv').length === 0;
			return a||b;
		}
