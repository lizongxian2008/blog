        ...
        return this.each(function () {
            ...
            var select = $('<div>',
							{ 'class': (settings.disabled ? settings.disabledclass : settings.selectclass),
							    ...
							    'click': function (evt) {
							        ...
							        $('div.' + settings.listboxclass, $this).css({'left': lbleft }).show();
							        stopBubble(evt);
            ...
            if (settings.checkbox) {
                ...
                $(_btn[0]).click(function () {
                    ...
                    if (objid.text.length > 5) {
                        objid.text = objid.text.replace("请选择;", "");
                        //objid.value = objid.value.replace(";", "");
                    }
                    if (objid.value == 'undefined' || objid.value == "") {
                        objid.text = "";
                    }
                    else {
                        if (objid.text.substring(0, 1) == ";") {
                            objid.text = objid.text.substring(1, objid.text.length);
                        }
                        if (objid.value.substring(0, 1) == ";") {
                            objid.value = objid.value.substring(1, objid.value.length);
                        }
                    }
            ...
            objid.hide = function (clear) {
                if (settings.checkbox && !clear) {
                    ...
                    $.each(tmpsv, function (i, n) {
                        var obj = $('input[value="' + n + '"]', listbox);
                        if (navigator.userAgent.toLowerCase().indexOf('msie 6.0') != -1) {
                            obj.attr('defaultChecked', 'true');
                        }
                        else {
